...
for session in ["DL", "RL", "BY"]:
    base_url = "https://nips.cc/Conferences/2017/Schedule?bySubject="
        ...
        if tag.has_attr("onclick"):
            if tag["onclick"].startswith("showDetail"):
                if config[year]["paperItem"] in tag.div["class"]:
                    paperList.append("https://nips.cc/Conferences/2018/Schedule?showEvent=" + tag["onclick"][11:-1])
        return False
    ...
    def isAuthor(tag):
        if tag.has_attr("onclick"):
            if tag["onclick"].startswith("showSpeaker"):
                speakerList.append("https://nips.cc/Conferences/2018/Schedule?showSpeaker=" + tag["onclick"][13:-3])
        return False
